...
const SeatChart = ({ occasion, nftToken, provider, setToggle }) => {
  const [seatsTaken, setSeatsTaken] = useState(false)
  const [hasSold, setHasSold] = useState(false)

  const getSeatsTaken = async () => {
    const seatsTaken = await nftToken.getSeatsTaken(occasion.id)
  ...
  const buyHandler = async (_seat) => {
    ...
    const transaction = await nftToken.connect(signer).mint(occasion.id, _seat, { value: occasion.cost })
